...
Last updated: Apr 23, 2026. Do not edit without team agreement.
...
│   ├── odi_scorer.py          # clusters + chunks → ODI scores (deterministic)
...
chunks: list[dict]     # output of chunker.py
# total_chunks is derived from len(chunks) — no extra arg needed

# Output — list of dicts, one per cluster, sorted by opportunity_score descending
[
  {
    "cluster_id": int,
    "cluster_size": int,
    ...
    "satisfaction": float,      # (avg_sentiment + 1) / 2, normalised to 0.0–1.0
    "opportunity_score": float  # importance * (1 - satisfaction), range 0.0–1.0
  },
  ...
]
...
# LLM generates — JTBD and solutions only, no priority_score
  ...
  "opportunities": [
    {
      ...
      "cluster_id": int,
      "importance": float,      # injected from scored_clusters, not LLM-generated
      "satisfaction": float,    # injected from scored_clusters, not LLM-generated
      "opportunity_score": float, # injected from scored_clusters, not LLM-generated
      "solutions": [
        {
          "label": str,
          "assumptions": [
...
# If a cluster_id from the LLM has no match in scored_clusters, set ODI fields to null — do not crash.
```
...
st.session_state["ost"]             # dict — merged OST JSON (LLM output + injected ODI scores)
...
- The JTBD format for opportunities is strictly: "When I [situation], I want to [motivation],
  so I can [outcome]."
- `opportunity_score` / ODI fields are **never generated by the LLM** — they are injected
  post-parse by merging with `scored_clusters` on `cluster_id`.
...
> implement the VADER sentiment averaging and opportunity score computation."
